...
impl GoodsRegistry {
    ...
    pub fn len(&self) -> usize { self.goods.len() }
    pub fn is_empty(&self) -> bool { self.goods.is_empty() }
...
mod tests {
    ...
    fn ids_resolve_to_expected_goods() {
        ...
        assert_eq!(registry.get(ids::MANUFACTURES).category, GoodCategory::Manufactured);
    ...
    fn europe_prices_reflect_export_economy() {
        ...
        assert!(registry.get(ids::SUGAR).europe_price_pesos
                > registry.get(ids::SUGAR).base_price_pesos);
        // Manufactures are cheaper in Europe (the source).
        assert!(registry.get(ids::MANUFACTURES).europe_price_pesos
